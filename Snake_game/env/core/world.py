...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.randint(SNAKE_SIZE + 1, self.size[0] - SNAKE_SIZE - 1),
                              random.randint(SNAKE_SIZE + 1, self.size[1] - SNAKE_SIZE - 1))
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = set(zip(*np.where(self.world != 255)))
        [self.current_available_food_positions.remove(x) for x in self.snake.blocks]
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.world[new_snake_head] == 255:
